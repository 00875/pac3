...
function pace.LoadPartsFromTable(data, clear, override_part)
	...
	if data.self then
		local part = override_part or pac.CreatePart(data.self.ClassName, nil, data, pac.GetPartFromUniqueID(pac.LocalPlayer:DLibUniqueID(), data.self.UniqueID):IsValid() and copy_id)
		...
		for key, tbl in pairs(data) do
			local part = pac.CreatePart(tbl.self.ClassName, nil, tbl, pac.GetPartFromUniqueID(pac.LocalPlayer:DLibUniqueID(), tbl.self.UniqueID):IsValid() and copy_id)
			table.insert(partsLoaded, part)
		end
	end
